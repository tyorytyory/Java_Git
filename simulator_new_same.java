...
public class simulator_new_same {//11/10更新
		...
		do{
			...
			do{
				...
				if(total > 7200 && total < 12600){//昼休み
					...
					for(int i=0;i<24;i++){
						if(total >=for_time_alpha && total<(for_time_alpha+900.0)){
							alpha = alpha_array[i];
					...
					//System.out.println(trade_time[i1]);
					i1++;
					/*if(c>0.9){
			...
			int bid_price = 15000;
			int ask_price = 15010;
			double limit_order_time = 1478;//秒単位(1000倍する)
			double limit_order_time_dif = 1478;//秒単位(1000倍する)
			double bid_or_ask;
			int bid_or_ask_Sfmt;
			...
			//try {
			//File file = new File("simulater_same" + count_pw + ".txt");//ファイルを一つ一つ出力する
			//PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(file)));
			...
			for(double d = 0;d<=for_time_order;d++){
				...
				if(d1 > 7200 && d1 <12600){//昼休み
					...
					bid_price = 15000;
					ask_price = 15010;
					if((limit_order_time/1000) == d1){
						limit_order_time +=limit_order_time_dif;
					}

				}
				else if((limit_order_time/1000) == d1){
